...
ip_local=$(ifconfig wlan0 | grep 'inet addr:' | cut -d: -f2 | awk '{ print $1}')
echo IP = $ip_local
...
sudo ip addr add 10.0.2.1 dev gre1
...
sudo ip addr add 10.0.4.1 dev gre2
...
sudo ip addr add 10.0.6.1 dev gre3
